...
var UserComponent = (function () {
    ...
    UserComponent.prototype.deleteUser = function (user_id) {
        var _this = this;
        if (confirm("Are you sure??")) {
            this.userService.delete(user_id).subscribe(function (user) {
                for (var i = 0; i < _this.allUser.length; i++) {
                    if (_this.allUser[i]._id == user_id)
                        _this.allUser.splice(i, 1);
                }
            });
        }
    };
